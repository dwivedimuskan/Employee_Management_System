...
const EmployeeLayout = ({ children }) => {
  ...
  const formatNotificationMessage = (message) => {
    if (!message) return '';
    console.log("Original message:", message); // <-- Log original message

    // Regex to find the full date-time-zone string, capturing the date part
    const fullDateRegex = /(\w{3}\s\w{3}\s\d{1,2}\s\d{4})\s\d{2}:\d{2}:\d{2}\sGMT[+-]\d{4}\s\(.+\)/g;

    const formattedMessage = message.replace(fullDateRegex, (match, datePart) => {
        console.log("Regex matched:", match);       // <-- Log if regex finds a match
        console.log("Captured datePart:", datePart); // <-- Log the captured group
        return datePart; // Replace with captured date
    });

    console.log("Formatted message:", formattedMessage); // <-- Log the result
                        ...
                        <>
                          {notifications.map((notification) => (
                            ...
                            >
                              ...
                              <p className="text-sm text-gray-500 mt-1">{formatNotificationMessage(notification.message)}</p>
